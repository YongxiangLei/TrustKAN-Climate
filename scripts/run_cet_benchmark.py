...
def build_model(name, history, horizon):
    ...
    if name in {"kan", "trustkan"}: return TrustKAN(1, horizon=horizon, hidden_dim=64, grid_size=8)
    raise KeyError(name)
...
def main(config):
    ...
    for horizon in cfg["window"]["horizons"]:
        ...
        # Remove boundary windows whose full forecast target crosses the region end.
        ...
        p=PersistenceForecaster(horizon).predict(test_x).numpy()
        rows.append({"model":"persistence","horizon":horizon,"seed":-1,"rmse_std":rmse(test_y,p),"mae_std":mae(test_y,p)})
        for name in [m for m in cfg["models"] if m not in {"persistence"}]:
            for seed in cfg["training"]["seeds"]:
                set_seed(seed)
                model=build_model(name,cfg["window"]["history"],horizon)
                tr=loader(*sets["train"],batch,True); va=loader(*sets["val"],batch); te=loader(*sets["test"],batch)
                model,hist,seconds=train_regressor(model,tr,va,epochs=cfg["training"]["epochs"],lr=cfg["training"]["learning_rate"],patience=cfg["training"]["patience"])
                pred,true=predict(model,te)
                np.savez_compressed(outdir/f"cet_{name}_h{horizon}_s{seed}.npz",prediction=pred,target=true)
                rows.append({"model":name,"horizon":horizon,"seed":seed,"rmse_std":rmse(true,pred),"mae_std":mae(true,pred),"train_seconds":seconds,"parameters":sum(p.numel() for p in model.parameters())})
    result=pd.DataFrame(rows)
    Path("results/aggregated").mkdir(parents=True,exist_ok=True)
    result.to_csv("results/aggregated/cet_runs.csv",index=False)
    summary=result.groupby(["model","horizon"],as_index=False).agg(rmse_mean=("rmse_std","mean"),rmse_sd=("rmse_std","std"),mae_mean=("mae_std","mean"),mae_sd=("mae_std","std"))
    summary.to_csv("results/aggregated/cet_summary.csv",index=False)
    print(summary.to_string(index=False))

if __name__ == "__main__":
    ap=argparse.ArgumentParser(); ap.add_argument("--config",default="configs/cet.yaml")
