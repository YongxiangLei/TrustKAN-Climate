...
This driver is intentionally generic: it expects train/validation/calibration/test
arrays prepared by a dataset-specific benchmark. It never tunes thresholds on test.
"""
...
def main(args):
    ...
    tr=loader(data['x_train'],data['y_train'],args.batch,True)
    va=loader(data['x_val'],data['y_val'],args.batch)
    ca=loader(data['x_cal'],data['y_cal'],args.batch)
    te=loader(data['x_test'],data['y_test'],args.batch)
    ...
    cal_lo,cal_hi=cal['quantiles'][...,lo_idx],cal['quantiles'][...,hi_idx]
    test_lo,test_hi=test['quantiles'][...,lo_idx],test['quantiles'][...,hi_idx]
    radius=conformal_radius(cal['target'],cal_lo,cal_hi,alpha=args.alpha)
    cal_lo_c,cal_hi_c=apply_conformal(cal_lo,cal_hi,radius)
    test_lo_c,test_hi_c=apply_conformal(test_lo,test_hi,radius)
    ...
    rel_cal=fuse_reliability(width_rel_cal,shift_rel_cal)
    rel_test=fuse_reliability(width_rel_test,shift_rel_test)
    ...
    mask=selective_mask(rel_test,selected['threshold'])
    cov_curve,risk_curve=risk_coverage_curve(test['target'],test['point'],np.repeat(rel_test,test['target'].shape[1]))
    result={
        'train_seconds':seconds,'conformal_alpha':args.alpha,'conformal_radius':radius,
        'calibration_coverage':interval_coverage(cal['target'],cal_lo_c,cal_hi_c),
        'test_coverage':interval_coverage(test['target'],test_lo_c,test_hi_c),
        ...
        'test_rmse_selected':rmse(test['target'][mask],test['point'][mask]) if mask.any() else None,
        'aurc':aurc(cov_curve,risk_curve),
    }
...
if __name__=='__main__':
    p=argparse.ArgumentParser(); p.add_argument('--split-file',required=True); p.add_argument('--out',default='results/reliability/trustkan.json')
    p.add_argument('--epochs',type=int,default=100); p.add_argument('--patience',type=int,default=10); p.add_argument('--lr',type=float,default=1e-3)
    p.add_argument('--batch',type=int,default=64); p.add_argument('--hidden',type=int,default=64); p.add_argument('--grid',type=int,default=8)
    p.add_argument('--alpha',type=float,default=.1); p.add_argument('--min-coverage',type=float,default=.5); main(p.parse_args())
