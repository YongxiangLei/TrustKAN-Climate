"""Plan a resumable, shardable Jena publication campaign."""
...
from scripts.run_jena_benchmark import combined_config_sha256, load_config, validate_config
...
def publication_checks(benchmark, reliability, benchmark_name, reliability_name):
    return {
        ...
        "shared_panel": benchmark["dataset"]["panel_config"]
        == reliability["dataset"]["panel_config"],
        "shared_prepared_dir": benchmark["dataset"]["prepared_dir"]
        == reliability["dataset"]["prepared_dir"],
    ...
    *,
    ...
    for horizon in reliability["window"]["horizons"]:
        for seed in reliability["training"]["seeds"]:
            tokens = [
                python,
                "scripts/run_jena_reliability.py",
                "--config",
    ...
    campaign = {
        ...
        "inputs": {
            ...
            "prepared_dir": str(benchmark["dataset"]["prepared_dir"]),
        },
        "matrix": {
...
def write_campaign(campaign, outdir):
    ...
    audit_command = _powershell(
        [
            python,
            "scripts/audit_jena_campaign.py",
            "--campaign",
...
if __name__ == "__main__":
    ...
    parser.add_argument("--benchmark-config", default="configs/jena.yaml")
    parser.add_argument("--reliability-config", default="configs/jena_reliability.yaml")
    parser.add_argument("--outdir", default="results/campaigns/jena_publication")
