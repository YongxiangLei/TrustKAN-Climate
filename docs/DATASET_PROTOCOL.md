...
Frozen protocol: download official 10-minute archives, quality-control
`T (degC)`, `p (mbar)`, `rh (%)` and `wv (m/s)`, and aggregate to hourly means
over 2010-01-01 00:00 through 2020-12-31 23:00. Derived thermodynamic columns
are excluded. Hours need at least four valid 10-minute slots per required
variable. Eligibility requires 10 years of span, 95% completeness against the
fixed window, and a maximum 48-hour gap. Model performance is never consulted.
Exact leakage controls are in `docs/JENA_BENCHMARK_PROTOCOL.md`.

Observed archive hashes and completeness are generated by
`scripts/audit_jena.py` and `scripts/prepare_jena.py` after the official files
are recorded in `data/manifests/jena_sources.csv`. Those observed numbers must
not be typed into this file until the eligibility audit exists.
...
`docs/GHCN_BENCHMARK_PROTOCOL.md`. The corresponding Jena design is in
`docs/JENA_BENCHMARK_PROTOCOL.md` and `docs/JENA_CAMPAIGN.md`.
